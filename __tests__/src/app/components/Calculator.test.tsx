...
const getAllByExactTextContent = (text: string) =>
    screen.getAllByText((_, element) => element?.textContent === text);
...
describe('Calculator', () => {
    ...
    it('初期値から負担割合と拠出額を表示する', () => {
        ...
        expect(getByExactTextContent('拠出額: 96,279円')).toBeInTheDocument();
        expect(screen.getByText('負担割合: 46.5%')).toBeInTheDocument();
        expect(getByExactTextContent('拠出額: 83,721円')).toBeInTheDocument();
        expect(getByExactTextContent('収入 - 拠出額: 133,721円')).toBeInTheDocument();
        expect(getByExactTextContent('収入 - 拠出額: 116,279円')).toBeInTheDocument();
    ...
    it('入力変更時に計算結果をリアルタイム更新する', () => {
        ...
        expect(getAllByExactTextContent('拠出額: 90,000円')).toHaveLength(2);
    });
    ...
    it('端数丸めが発生しても拠出額合計が予算合計と一致する', () => {
        ...
        expect(getByExactTextContent('拠出額: 1円')).toBeInTheDocument();
        expect(getByExactTextContent('拠出額: 0円')).toBeInTheDocument();
    });
});
